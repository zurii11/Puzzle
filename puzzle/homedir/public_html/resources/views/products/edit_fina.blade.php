								...
								<div class="col-lg-7">
									<select class="form-control demo-select2-placeholder" name="fina_category" id="fina_category" required>
										@foreach($groups->groups as $groupslist)
											<option value="{{$groupslist->id}}" @php if($groupslist->id == $product->finacategory_id) { echo 'selected'; } @endphp >{{__($groupslist->name)}}</option>
										@endforeach
									</select>
								</div>
							</div>
								...
								<div class="col-lg-7">
									<select class="form-control demo-select2-placeholder" name="unit" required>
										@foreach($units->units as $unitslist)  
											<option value="{{$unitslist->id}}" @php if($unitslist->id == $product->finaunit_id) { echo 'selected'; } @endphp>{{__($unitslist->full_name)}}</option>
										@endforeach
									</select>
								</div>
							</div>
