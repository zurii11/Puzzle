        ...
        if(Auth::check()){
            if(Auth::user()->status == '2'){
                if($product->unit_price_s1_type == 'amount'){
                    $lowest_price =  $product->unit_price_s1;  
                    $highest_price =  $product->unit_price_s1;  
                }elseif($product->unit_price_s1_type == 'percent'){
                    $lowest_price = $product->unit_price - ($product->unit_price*$product->unit_price_s1/100) ;
                    $highest_price = $product->unit_price - ($product->unit_price*$product->unit_price_s1/100) ;
                }
            }elseif(Auth::user()->status == '3'){
                if($product->unit_price_s2_type == 'amount'){
                    $lowest_price = $product->unit_price_s2;  
                    $highest_price = $product->unit_price_s2;  
                }elseif($product->unit_price_s2_type == 'percent'){
                    $lowest_price = $product->unit_price - ($product->unit_price*$product->unit_price_s2/100) ;
                    $highest_price = $product->unit_price - ($product->unit_price*$product->unit_price_s2/100) ;
                }
            }
        }
